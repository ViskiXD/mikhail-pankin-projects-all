...
public class SpaceEchoHandle {
    ...
    [DllImport("SpaceEcho", CallingConvention = CallingConvention.StdCall)]
    ...
    internal SpaceEchoHandle(int key) {
        PluginKey = key;
    }
    ...
    private static SpaceEchoHandle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (SpaceEchoHandle)gch.Target;
    }
...
public class SpaceEchoHelper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static SpaceEchoHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("SpaceEchoHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                SpaceEchoHelper helper = go.AddComponent<SpaceEchoHelper>();
                helper.Plugin = new SpaceEchoHandle(key);
                return helper;
            }
            return go.GetComponent<SpaceEchoHelper>();
        }
    }

    public SpaceEchoHandle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            SpaceEchoHandle.RegisterGlobalTransport(null);
        }
    }
}
